...
echo "<input id='clubid' type='hidden' hidden value='".$_GET['id']."'>";

if(isset($_GET['id']))
{
  $club       = Club::get_by_id($_GET['id']);
  ...
  $clubUsers  = ClubUser::getStudentsInClub($club->id);

  if($session->is_logged_in())
  {
    if(!User::get_by_id($session->user_id)->is_super_admin())
    {
      if($club->pending == 1 || $club->enabled == 0)
      {
        header("location: index.php?negative");
      }
    }
...
<div class="container-fixed">
  <div class="row-fixed">
    <div class="span10 offset2">
      ...
      <div class="span9 box">
        ...
        <div id="myCarousel" class="carousel slide">
          ...
          <div class="carousel-inner">
            <div class="active item">
              <?php echo "<img class='img-polaroid' style='width:98%;' src='data:image/jpeg;base64, ".$club->cover."' />"; ?>
            </div>
          </div>

          <div class="carousel-caption">
            <?php echo "<img src='data:image/jpeg;base64, ".$club->logo."' />"; ?>
          </div>
        ...
        <!-- /Carousel -->
        <span class="myname"><?php echo $club->name; ?></span>
        ...
        <div class="row-fixed" >
          <div class="span9" style="margin:0px; padding:0px;">
            ...
            if(count($clubUsers) > 0)
            {
              ...
              {
                if($clubUser->pending == 0 && $clubUser->enabled == 1)
                {
                  $user = User::get_by_id($clubUser->userid);
  ...
  { 
    ...
    function loadComments()
    {
      ...
      {
        type: 'GET',
        url: 'includes/webservices/getcomments.php?itemid='+$("#clubid").val()+'&itemtype=club',
    ...
    function addComment(thecomment, element)
    {
      ...
      {
        type: 'POST',
        data: {comment: thecomment, itemid: $("#clubid").val(), itemtype: "club"},
    ...
    function loadStudents()
    {
      ...
      {
        type: 'GET',
        url: 'includes/webservices/filterstudents.php?clubid='+$("#clubid").val()+'&input=' + $("#studentfilter").val(),
        success: function(result)
        {
    ...
    function invite(userid, orgid, element)
    {
      ...
      {
        type: 'GET',
        url: 'includes/webservices/invite.php?clubid='+orgid+'&userid='+userid,
        success: function(result) 
        {
          if(result == "success")
          {
    ...
    function join()
    {
      ...
      {
        type: 'GET',
        url: 'includes/webservices/join.php?clubid=<?php echo $club->id; ?>',
        success: function(result) 
        {
          if(result == "success")
          {
    ...
    function optout()
    {
      ...
      {
        type: 'GET',
        url: 'includes/webservices/optout.php?clubid=<?php echo $club->id; ?>',
        success: function(result) 
        {
          if(result == "success")
          {
            bootbox.alert("<b><i>You are now out in the club.</i></b> <br /><br />");
            window.location.reload();
          }
          else
          {
